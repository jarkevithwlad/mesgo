...
import { state, getActiveAccount, getDialogMap, getPeerRuntime, getSelectedPeerGuid, clearPeerRuntime } from './state.js';
import { MESSAGE_NAMESPACE, WEBRTC_RETRY_INTERVAL, nowSeconds, uuidV5 } from './utils.js';
import { ensureDialog, receiveDirectChatMessage } from './chats.js';
...
async function buildSignalMessage(type, payload) {
  ...
  return {
    guid: (await uuidV5(MESSAGE_NAMESPACE, `${timestamp}|signal|${type}|${Math.random()}`)).toLowerCase(),
    timestamp,
    type,
...
export async function ensurePeerConnection(peerGuid, peerNickname) {
  ...
  const runtime = getPeerRuntime(peerGuid);
  if (runtime.pc) return runtime.pc;
