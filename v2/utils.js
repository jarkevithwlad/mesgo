...
export const SIGNAL_NAMESPACE = '6ba7b813-9dad-11d1-80b4-00c04fd430c8';
export const SCROLL_BOTTOM_THRESHOLD = 48;
export const POLL_INTERVAL = 2500;
export const WEBRTC_RETRY_INTERVAL = 8000;
export const HANDSHAKE_INTERVAL = 5000;
export const MSG_ACK_TIMEOUT = 3000;
export const MSG_ACK_RETRIES = 3;
...
/**
 * Формирует уникальный "book" URL для пары клиентов.
 * Каждое сочетание sender+receiver даёт свой уникальный path,
 * чтобы сообщения разных диалогов не пересекались.
 */
export function makeSignalBookPath(senderGuid, receiverGuid) {
  const sorted = [String(senderGuid).toLowerCase(), String(receiverGuid).toLowerCase()].sort();
  return `/messages_v1/book_${sorted[0]}_${sorted[1]}`;
}

/**
 * Формирует STUN book path для пары.
 */
export function makeStunBookPath(senderGuid, receiverGuid) {
  const sorted = [String(senderGuid).toLowerCase(), String(receiverGuid).toLowerCase()].sort();
  return `/messages_v1/stun_book_${sorted[0]}_${sorted[1]}`;
}
