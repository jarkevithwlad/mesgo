...
function renderDeliveryStatusIcon(message) {
  ...
  switch (status) {
    case 'pending':
      return '<span class="delivery-status" title="Ожидает отправки">&#10003;</span>';
    case 'sent':
      return '<span class="delivery-status sent" title="Отправлено">&#10003;</span>';
    case 'sending':
      return '<span class="delivery-status sending" title="Отправка...">&#9203;</span>';
    case 'delivered':
      return '<span class="delivery-status delivered" title="Доставлено">&#10003;&#10003;</span>';
    case 'error':
      return '<span class="delivery-status error" title="Ошибка отправки">&#9888;</span>';
    case 'retrying':
      return '<span class="delivery-status retrying" title="Повторная отправка...">&#9203; retry</span>';
    default:
      return '<span class="delivery-status" title="Отправлено">&#10003;</span>';
  }
}
...
function renderMessageHtml(dialog, message) {
  ...
  const deliveryIcon = renderDeliveryStatusIcon(message);
...
async function handleSendFromComposer() {
  ...
  try {
    const result = await sendMessageToPeer(dialog.peerGuid, dialog.peerNickname, text, { preferDirect: true });
    els.composerInput.value = '';

    if (result.direct) {
      els.composerStatus.className = 'composer-status status-ok';
      els.composerStatus.textContent = 'Отправлено (P2P).';
    } else {
      els.composerStatus.className = 'composer-status status-ok';
...
function bindEvents() {
  ...
  els.accountsList?.addEventListener('click', async (event) => {
    ...
    if (deleteBtn) {
      ...
      const ok = window.confirm(`Удалить аккаунт ${accountId}?`);
      if (!ok) return;
